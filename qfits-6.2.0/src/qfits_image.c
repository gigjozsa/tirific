...
/**
  ...
  @param    bscale      FITS BSCALE in original file.
  @param    bzero       FITS BZERO in original file.
  @param    goal        goal array
  @return   0: success
            1: error

  This function takes in input a pointer to a byte buffer as given in
  the original FITS file (big-endian format). It converts the buffer
  to an array of float (whatever representation is used for floats by
  this platform is used) and puts it into the buffer goal, which must
  be large enough.

 */
/*----------------------------------------------------------------------------*/
int qfits_pixin_floatbuf(
        byte    *   p_source,
        int         npix,
        int         bitpix,
        double      bscale,
