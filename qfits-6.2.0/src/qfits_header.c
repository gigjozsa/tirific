...
char * qfits_header_getline(qfits_header * hdr, char * key)
{
	...
	/* GJ This is cheating; we recreate lin on the spot (on the other hand, I would not know where the harm is) */	
	if (k -> lin == NULL) {
	  k->lin = (char *) malloc(81*sizeof(char));
	}
	memset(k -> lin, 0, 81);
	memset(k -> lin, ' ', 80);

	qfits_header_makeline(k -> lin, k, 0);
